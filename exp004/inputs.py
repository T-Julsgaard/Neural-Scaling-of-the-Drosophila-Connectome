...
from exp003.growth import (ARMS, SIZES, Base, Graph, allocation, degree_shuffle, digest,
                     from_record as validation_rebuild, resample)

STAGES = {"validation": (0, range(5)), "development": (1, range(100, 104)), "evaluation": (2, range(1000, 1032))}
...
def stream(stage, block, domain, component, *suffix, replicate=0, size=0):
    ...
    return np.random.Generator(np.random.PCG64(np.random.SeedSequence(
        [5, STAGES[stage][0], block, domain, replicate, size, component, *suffix])))
...
def rebuild(base, record):
    ...
    if record["seed_namespace"] != [5, STAGES[stage][0], block, 1, rep, size]:
...
def graphs_for(base, stage, block, size, replicate):
    ...
    for arm, columns in added.items():
        ...
        record = {"schema": 2, "stage": stage, "block": block, "replicate": replicate, "size": size,
                  ...
                  "seed_namespace": [5, STAGES[stage][0], block, 1, replicate, size],
...
def encode_fast(graph, observations, priority, active_count=4, bottleneck=False):
    p, u = graph.projection, np.asarray(observations)
    n = p.shape[1]
    if active_count not in (4, 6) or u.shape[-1] != 40:
...
class EpisodeTask:
    def __init__(self, inputs, graph, mode="full_k4"):
        if mode not in ("full_k4", "full_k6", "bottleneck_k4", "bottleneck_k6"):
    ...
    def encode(self, observations, bottleneck=None):
        return encode_fast(self.graph, observations, self.priority,
                           int(self.mode[-1]),
                           self.mode.startswith("bottleneck") if bottleneck is None else bottleneck)
...
def conditions(base, stage, block):
    rows = []
    for size in (73, 110):
        for rep in range(1 if size == 73 else 3):
            graphs = graphs_for(base, stage, block, size, rep)
            for arm in (("clone",) if size == 73 else ("structured", "degree_null")):
                for readout in ("full", "bottleneck"):
                    for k in (4, 6):
                        mode = f"{readout}_k{k}"
                        rows.append({"id": f"N{size}_r{rep}_{arm}_{mode}", "size": size,
                                     "replicate": rep, "arm": arm, "mode": mode, "graph": graphs[arm]})
    if len(rows) != 28 or len({r["id"] for r in rows}) != 28:
