...
import numpy as np
from exp007.campaign import ROOT, folder, read, write, interval
...
def main():
    ...
    for b in c['blocks']:
        with np.load(out/f'block_{b}.npz') as a:
            vals = []
            for m in ('homeostasis', 'baseline6'):
                prefix = f'c11_{m}_g0_r0_'; counts = a[prefix+'presented']; diag = a[prefix+'diagnostics'][0]
    ...
    write(out/'geometry_contrasts.json', dict(scope='Prespecified diagnostics; descriptive paired 95% intervals, not causal mediation or new primary tests', contrast='homeostasis minus uncalibrated K6; load16/overlap6/128 per pair', values=values, source_sha256=sha256(Path(__file__))))
    print(values)
