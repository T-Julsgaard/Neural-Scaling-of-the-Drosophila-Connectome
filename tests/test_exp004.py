...
from exp004.engine import execute_condition, retained_bytes


class ActivityDiagnosticTests(unittest.TestCase):
    def test_storage_scan_tolerates_concurrent_atomic_rename(self):
        stable, vanished = Mock(), Mock()
        stable.is_file.return_value = vanished.is_file.return_value = True
        stable.stat.return_value.st_size = 123
        vanished.stat.side_effect = FileNotFoundError('Atomic rename')
        with patch('pathlib.Path.rglob', return_value=iter([stable, vanished])):
            self.assertEqual(retained_bytes('unused'),123)
        vanished.stat.side_effect = PermissionError('Unexpected access failure')
        with patch('pathlib.Path.rglob', return_value=iter([vanished])):
            with self.assertRaises(PermissionError):
                retained_bytes('unused')
