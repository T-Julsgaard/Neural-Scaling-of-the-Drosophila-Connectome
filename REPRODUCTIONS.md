...
Updated 2026-09-11. **All published scientific reproductions are NOT RUN.** R02's automated equation-adaptation checks now pass, with independent review/author runtime pending. Downloading source, reading methods and auditing a matrix are not reproductions.
...
| R02 | Bennett eq8 mechanism | MATLAB `mb_mv_a.m` pinned in software audit; fixed provided activity/choice/outcome trace, 256 updates | Per-step q/d/weights ≤10^-10. If author runtime unavailable, independently reviewed analytic validation is labelled adaptation validation, not reproduction | Automated adaptation checks PASS (max error 4.44e-16); independent review pending; author runtime NOT RUN. [Evidence](experiments/EXP-002-validation.md) |
