"""Fresh EXP-003 streams and campaign graph/task adapters; frozen R03 is untouched."""
import copy
...
def stream(stage, block, domain, component, *suffix, replicate=0, size=0):
    if stage not in STAGES or block not in STAGES[stage][1]:
        raise ValueError("Stage/block outside development runner contract")
...
def grid():
    return [Config("native", "delta", eta, temp) for eta in (.0001, .001, .01) for temp in (.1, .2, .5)]
...
def select_shared(block_rows):
    """Each block has equal weight; each of its 5x4 main cells has equal weight."""
    per_block = []
    for rows in block_rows:
        cells = []
        for size in SIZES:
            for arm in ARMS:
                matching = [r for r in rows if r["size"] == size and r["mode"] == "full" and
                            r["arm"] == ("clone" if size == 73 else arm)]
                if len(matching) != (1 if size == 73 else 3):
                    raise ValueError("Missing or duplicated selection cell")
                cells.append(np.mean([r["metrics"]["selection_score"][:9] for r in matching], axis=0))
        per_block.append(np.mean(cells, axis=0))
    scores = np.mean(per_block, axis=0)
    candidates = grid()
    tied = [i for i, s in enumerate(scores) if scores.max() - s <= 1e-6]
    selected = min(tied, key=lambda i: (candidates[i].eta, -candidates[i].temperature))
    return {"index": selected, "config_id": candidates[selected].id, "scores": scores.tolist(),
            "block_scores": np.asarray(per_block).tolist()}
