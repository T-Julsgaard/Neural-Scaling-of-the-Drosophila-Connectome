...
import numpy as np
from exp007.memory import rng, prototypes, random_projection, digest, task, probability, probe
...
def candidates(method):
    if method in ('direct', 'oracle'):
        return [dict(k=0, strength=0., eta=float(e), temperature=t) for e in np.geomspace(ETAS[0], .015, 16) for t in TEMPS]
    pairs = [(4, 0.), (6, 0.)] if method.endswith('ordinary') else [(6, .25), (6, .5)]
    return [dict(k=k, strength=l, eta=e, temperature=t) for k, l in pairs for e in ETAS for t in TEMPS]
...
def encode(u, p, priority, k, theta):
    ...
    ix = np.lexsort((np.broadcast_to(priority, z.shape), -z), axis=-1)[..., :k]
    x = np.zeros_like(z)
...
def calibration_pool(validation=False):
    ...
    for overlap in (2, 6):
        for j in range(64):
            ...
            p, _ = prototypes('validation', number, overlap)
            noise = rng('validation', number, 90, overlap).normal(0, .1, p.shape)
            pool.append(np.clip(p+noise, 0, 1).reshape(-1, 40))
...
def fit(p, u, priority, strength):
    ...
    bound = .25*scale
    theta = np.zeros(73)
    p0 = (encode(u, p, priority, 6, theta) > 0).mean(0)
    target = (1-strength)*p0+strength*6/73
    history = []
    for _ in range(100):
        current = (encode(u, p, priority, 6, theta) > 0).mean(0)
        history.append(float(np.mean((current-target)**2)))
        theta = box_center(theta+.5*scale*(current-target), bound)
    final = (encode(u, p, priority, 6, theta) > 0).mean(0)
...
def shifted(t):
    out = dict(t)
    factor = np.r_[np.full(20, .5), np.ones(20)]
    out['p'] = t['p']*factor
