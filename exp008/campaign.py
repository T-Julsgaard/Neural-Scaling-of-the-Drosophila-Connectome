"""Prospectively frozen EXP-008 execution, audit and paired inference."""
...
from exp008.homeostasis import *

CONDITIONS = [(s, r, l, False) for s in (LOW, HIGH) for r in ('per_pair', 'total') for l in (4, 16)]
SHIFT = []
REPORT = ('baseline_high', 'ordinary', 'homeostasis', 'sham', 'random_ordinary', 'random_homeostasis', 'direct', 'oracle', 'fixed_homeostasis', 'fixed_sham', 'matched_sham')


def folder(stage): return ROOT/f'results/exp008_{stage}'


def snapshot():
    paths = [*ROOT.glob('exp008/*.py'), *ROOT.glob('exp002/*.py'), *ROOT.glob('exp006/*.py'), *ROOT.glob('exp007/*.py'), ROOT/'tests/test_exp008.py', ROOT/'tools/run_transfer.py', ROOT/'experiments/EXP-008-protocol.md']
...
def prepare():
    ...
    if (out/'record.json').exists():
        ...
        return
    start = time.perf_counter(); before_source = snapshot(); p, provenance = load_projection()
    assert p.shape == (D, N)
    priority = rng('validation', 9000, 7).permutation(N)
    ...
    for j, projection in enumerate(projections):
        for strength in (.25, .5):
            ...
            rec['validation_before'] = (encode(valid, projection, priority, KHIGH, np.zeros(N)) > 0).mean(0).tolist()
            rec['validation_after'] = (encode(valid, projection, priority, KHIGH, theta) > 0).mean(0).tolist()
            arrays[key] = theta; records[key] = rec
            if j == 0:
                for rep in range(3): arrays[f'sham{rep}_l{strength}'] = theta[rng('validation', 9000, 9, rep).permutation(N)]
    assert before_source == snapshot()
...
def specs(stage, selection=None):
    ...
    result['baseline_high'] = [selected['baseline_high']]
    result['matched_sham'] = [selected['homeostasis']]
    for label in ('homeostasis', 'sham'):
        result['fixed_'+label] = [{**selected[label], 'eta': selected['baseline_high']['eta'], 'temperature': selected['baseline_high']['temperature']}]
...
def compute(stage, number, settings):
    ...
    for ci, (s, regime, load, shift) in enumerate(conditions):
        ...
        arrays[key+'_input_overlap'] = t['p'].reshape(-1, D) @ t['p'].reshape(-1, D).T
        ...
        for method, configs in settings.items():
            ...
            for gi, ((k, strength), indices) in enumerate(groups.items()):
                ...
                for rep in range(reps):
                    ...
                    theta = np.zeros(N) if strength == 0 else cal[f'sham{rep}_l{strength}' if 'sham' in method else f'p{pj}_l{strength}']
...
def verify(c):
    ...
    assert read(folder('calibration')/'record.json')['source'] == snapshot()
    load_projection()
    if c['stage'] == 'confirmation':
        assert c['selection_sha256'] == sha256(folder('development')/'selection.json')
...
def freeze(stage):
    ...
    v = read(ROOT/'research/exp008_validation.json')
...
def block(stage, b):
    ...
    rec.update(completed_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), sha256=sha256(path), contract_sha256=sha256(out/'contract.json'))
    write(record, rec)
...
def interval(v, coverage=.95, draws=50000, lower_alpha=None):
    v = np.asarray(v); indices = np.random.default_rng(800891).integers(len(v), size=(draws, len(v)))
...
def analyze(stage):
    ...
    if stage == 'development':
        ...
        base = [i for i, v in enumerate(c['settings']['ordinary']) if v['k'] == KHIGH]
        chosen['baseline_high'] = base[int(np.argmax(score[0, base]))]
        selected = {m: c['settings'][m][i] for m, i in chosen.items() if m != 'baseline_high'}
        selected['baseline_high'] = c['settings']['ordinary'][chosen['baseline_high']]
        h = data[:, 5, methods.index('homeostasis'), chosen['homeostasis'], 0]
        contrasts = [h-data[:, 5, methods.index(m), chosen[m], 0] for m in ('ordinary', 'sham')]
        contrasts.append(h-data[:, 5, 0, chosen['baseline_high'], 0])
    ...
    primary = {}; guards = {}
    for m in ('baseline_high', 'ordinary', 'sham'):
        delta = data[:, 5, idx['homeostasis']]-data[:, 5, idx[m]]
    ...
    for ci, condition in enumerate(CONDITIONS+SHIFT):
        ...
        profiles[key]['contrasts'] = {m: {name: interval(data[:, ci, idx['homeostasis'], mi]-data[:, ci, idx[m], mi]) for mi, name in enumerate(NAMES)} for m in ('baseline_high', 'ordinary', 'sham', 'direct')}
        profiles[key]['fixed_contrasts'] = {m: {name: interval(data[:, ci, idx[m], mi]-data[:, ci, idx['baseline_high'], mi]) for mi, name in enumerate(NAMES)} for m in ('fixed_homeostasis', 'fixed_sham', 'matched_sham')}
    hard = profiles['_'.join(map(str, CONDITIONS[5]))]
    adequacy = dict(ordinary_new_learning=hard['ordinary']['new_learning']['interval'][0] > .80,
        oracle_retention=hard['oracle']['retention']['interval'][0] > .95,
        oracle_new_learning=hard['oracle']['new_learning']['interval'][0] > .95,
        below_ceiling=hard['ordinary']['retention']['interval'][1] < .95,
        above_floor=hard['ordinary']['retention']['interval'][0] > .55)
    adequacy['passed'] = all(adequacy.values())
    lo, hi = primary['ordinary']['interval']
    classification = ('equivalent' if lo > -.03 and hi < .03 else 'useful_benefit' if lo > .03 else 'harm' if hi < -.03 else 'inconclusive')
    supplemental = {name: interval(data[:,5,idx['homeostasis'],mi]-data[:,5,idx['matched_sham'],mi]) for mi,name in enumerate(NAMES)}
    write(out/'analysis.json', dict(primary=primary, guardrails=guards, profiles=profiles,
        informative_gate=adequacy, transfer_classification=classification,
        informative_transfer=classification == 'equivalent' and adequacy['passed'],
        exact_multiset_contrasts=supplemental, blocks=len(c['blocks']), contract_sha256=sha256(out/'contract.json')))

    print(json.dumps(dict(primary=primary, guardrails=guards)), flush=True)


def validate():
    suite = unittest.defaultTestLoader.loadTestsFromName('tests.test_exp008')
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    assert result.wasSuccessful()
    write(ROOT/'research/exp008_preflight.json', dict(status='passed', tests=result.testsRun, source=snapshot(), created_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())))
    prepare()
    ...
    write(ROOT/'research/exp008_validation.json', rec)
